...
export class SignalEngine {
  ...
  private wallets = new Map<string, WalletProfile>(); private median = new Map<string, number>();
  constructor(d: EngineDeps) { this.db = d.db; this.cfg = d.cfg ?? configFromEnv(); this.ch = d.channels ?? channelsFromEnv(); this.now = d.now ?? (() => Math.floor(Date.now() / 1000)); this.log = d.log ?? (() => {}); }
  ...
  async ingest(f: Fill): Promise<Signal[]> {
    ...
    for (const s of signals) {
      ...
      if (error) { if (error.code === "23505") continue; throw error; } // 23505 = duplicate dedupe key
      fired.push(s);
