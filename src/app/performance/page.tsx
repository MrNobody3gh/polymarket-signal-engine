...
import { currentReturn, pct, usd, MIN_SAMPLE, type Stats } from "@/lib/paper/analytics";
import { readSnapshot } from "@/lib/paper/snapshot";
...
export default async function Performance({ searchParams }: { searchParams: Promise<{ window?: string }> }) {
  ...
  const w = snap?.windows[days === 7 ? "d7" : days === 30 ? "d30" : "all"];
  const { rows, marks } = await loadPaper(db(), { sinceIso: days ? new Date(Date.now() - days * 86_400_000).toISOString() : undefined, limit: 300 });
  if (!snap || !w) return <><h1>Paper performance</h1><div className="empty">Snapshot not built yet — the worker rebuilds it after every marking run.</div></>;
  const st = w.stats;
  const wallets = Object.entries(snap.wallets).map(([key, v]) => ({ key, name: v.name, stats: v.stats })).sort((a, b) => b.stats.signals - a.stats.signals);
  return (
    <>
      <h1>Paper performance</h1>
      <p className="lede">Every signal is recorded as a ${rows[0]?.size_usd ?? 100} hypothetical long on the outcome token at the signal price, then marked at 1h / 6h / 24h and settled at resolution or when the wallet exits. Measured results only — nothing here is a forecast. Samples under {MIN_SAMPLE} are flagged. Aggregates rebuilt {new Date(snap.builtAt).toUTCString().slice(17, 22)} UTC; history shows the latest 300 signals.</p>
      ...
      <div className="wrap"><table>{HEAD}<tbody>{w.byKind.map((k) => <StatsRow key={k.key} label={KIND[k.key] ?? k.key} s={k.stats} />)}</tbody></table></div>
      <h2>By wallet copy score at signal time</h2>
      <div className="wrap"><table>{HEAD}<tbody>{w.byScoreBand.map((k) => <StatsRow key={k.key} label={k.key} s={k.stats} />)}</tbody></table></div>
      <h2>By consensus depth (consensus signals only)</h2>
      <div className="wrap"><table>{HEAD}<tbody>{w.byConsensusDepth.map((k) => <StatsRow key={k.key} label={k.key} s={k.stats} />)}</tbody></table></div>
      {!w.byConsensusDepth.length && <div className="empty">No consensus signals in this window.</div>}
      <h2>By wallet</h2>
      <div className="wrap"><table>{HEAD}<tbody>{wallets.slice(0, 100).map((x) => <StatsRow key={x.key} label={x.name ?? x.key.slice(0, 10)} s={x.stats} link={`https://polymarket.com/profile/${x.key}`} />)}</tbody></table></div>
      <h2>Signal history (latest 300)</h2>
